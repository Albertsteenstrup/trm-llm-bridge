...
def filter_null_grads(grads, opt, opt_state, opt_state_skeleton, algo_idx):
  ...
  # Older JAX treated None as a tree-prefix when flattening optimizer inputs.
  # Current JAX/Optax requires the gradient tree to match optimizer state
  # exactly, so represent filtered gradients as explicit zero arrays.
  zeroed_grads = jax.tree_util.tree_map(
      lambda masked, original: jnp.zeros_like(original) if masked is None else masked,
      masked_grads,
      grads,
      is_leaf=lambda x: x is None,
  )

  # Compute updates on the full optimizer tree, then explicitly suppress
  # updates for parameters filtered out for this algorithm.
  updates, new_opt_state = opt.update(zeroed_grads, opt_state)
  updates = jax.tree_util.tree_map(
      lambda masked, update: jnp.zeros_like(update) if masked is None else update,
      masked_grads,
      updates,
      is_leaf=lambda x: x is None,
  )
